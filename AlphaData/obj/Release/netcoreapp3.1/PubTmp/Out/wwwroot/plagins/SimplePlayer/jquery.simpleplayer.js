    ...
    $.fn.player = function(settings) {
        ...
        this.each(function () {
            ...
            $(this).parent().find('.simple-player-container').prepend(
                    '<a class="element_player start" href="javascript:void(0)">' + playControl + '</a>' + 
                    '<li class="progressbar-wrapper">' + 
                        '<span style="display: block; background-color: ' + config.progressbarBGColor + '; width: 100%; ">' + 
                            '<span class="progressbar" style="display: block; background-color: ' + config.progressbarColor + '; height: ' + config.progressbarHeight + '; width: 0%; "></span>' +
                        '</span>' + 
                    '</li>' + 
                    '<span class="element_player time"></span>' +
                '<a class="element_player download" href="' + simplePlayer.getAttribute('src') + '"><span class="simpleplayer-download-control"></span></a>'
            );

            
            var button = $(this).parent().find('.element_player.start');
            var progressbarWrapper = $(this).parent().find('.progressbar-wrapper');
            var progressbar = $(this).parent().find('.progressbar');
            var timeCurrent = $(this).parent().find('.element_player.time');
